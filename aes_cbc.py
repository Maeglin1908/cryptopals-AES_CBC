...
def matrixToBytes(data: np.matrix) -> bytes:
    return data.getT().tobytes()[::8]
...
def encryptBlock(data: np.matrix, key: np.matrix) -> np.matrix:
    ...
    return matrixToBytes(encoded)


def decrypt(data: bytes, key: bytes, iv: bytes, verbose: bool = False) -> bytes:
    if verbose:
        print("Decrypt mode")
        print("Data : \n{}\n".format(data))
        print("Key :\n{}\n".format(key))
    ...
    for i, c in reversed(list(enumerate(chunks))):
        m_cipher = bytesToMatrix(c, 4)
        inv_iv = bytesToMatrix(iv if i == 0 else chunks[i-1], 4)
        m_decrypted_block = (bytesToMatrix(decryptBlock(m_cipher, m_key), 4) ^ inv_iv).getT()
        decrypted_block = matrixToBytes(m_decrypted_block)
        decrypted = decrypted_block + decrypted
    return pkcsPadClean(decrypted)


def encrypt(data: bytes, key: bytes, iv: bytes, verbose : bool = False) -> bytes:
    if verbose:
        print("Encrypt mode")
        print("Data : \n{}\n".format(data))
        print("Key :\n{}\n".format(key))
    ...
    encrypted = b""
    m_key = bytesToMatrix(key, 4)
    m_iv = bytesToMatrix(iv, 4)
    previous_block = 0
    for i in range(len(chunks)):
        if i == 0:
            m_plain = bytesToMatrix(chunks[i], 4) ^ m_iv
        else:
            m_plain = bytesToMatrix(chunks[i], 4) ^ bytesToMatrix(previous_block, 4)
        encrypted_block = encryptBlock(m_plain, m_key)
...
def pkcsPad(data: bytes, bs: int) -> bytes:
    diff = bs - (len(data) % bs)
    data += bytes([diff]) * diff
    return data


def pkcsPadClean(data: bytes) -> bytes:
    nb = int(data[-1])
    return data[:-nb]


def usage():
    print("""Usage of AES-CCB
    Example :
    {} -k "Thats my Kung Fu" -i "initial vectoriz" -d "Two One Nine Two" -o output_file.txt
    {} -k key_file.txt -i iv_file.txt -d data_file.txt --decrypt
    ...
    -i (Required) : File containingg the IV, or the IV between qutes.
                    If it's not equal to 16 bytes, it will be truncated or padded.
    ...
    --kmode/--imode/--dmode (Optional) : raw/b64/hex : Specify the data-type on input
...
def main():
    ...
    try:
        ...
        if arg_key == "" or arg_data == "" or arg_iv == "":
            raise Exception("Key, IV and data required !")
    ...
    if os.path.exists(arg_iv):
        arg_iv = open(arg_iv, 'rb').read()
    else:
        arg_iv = arg_iv.encode()

    arg_data = readInput(arg_data, dmode)
    arg_key = readInput(arg_key, kmode)
    arg_iv = readInput(arg_iv, imode)
    ...
    if len(arg_iv) != bs:
        if len(arg_iv) < 16:
            print("IV length is shorter than {}. It will be padded.".format(bs))
            arg_iv += b'\x00' * (bs - arg_iv)
        else:
            print("IV length is longer than {}. It will be truncated".format(bs))
            arg_iv = arg_iv[:bs]

    if decryption:
        res = decrypt(arg_data, arg_key, arg_iv, verbose)
    else:
        res = encrypt(arg_data, arg_key, arg_iv, verbose)
    if output_file != "":
